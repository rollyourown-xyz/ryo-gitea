...
#--------------------------------------------------------------------------
# Packer template for building LXD container image for ryo-gitea
...
# Specify the Gitea version to use in the image build
variable "gitea_version" {
  description = "Mandatory: The gitea version to use in the image build."
  type        = string
}
...
locals {
  ...
  build_playbook_file  = "${abspath(path.root)}/playbooks/provision-gitea.yml"
  build_extra_vars     = "host_id=${var.host_id} gitea_version=${var.gitea_version}"
}
